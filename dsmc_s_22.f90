...
! 2016.8.3  プログラムを修正（コメントを加えたり，if文を減らしたり）
!
!---------------------------------------------------------------------------
...
program dsmc
  ...
  write(fileknu,'("KnudsenConf(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
     ...
     do incx = 1,iCellNumber1
        ...
        do j=1,icellPN(incx,incy)
           ...
           if(x(1,j,incx,incy)<=L1 .and. x(1,j,incx,incy)>=-L1) then
              ...
              !-- 粒子が所属するセルを決定範囲は[ ) --
              iincx = ceiling((x(1,j,incx,incy)+L1)*Redx1)   
              iincy = ceiling((x(2,j,incx,incy)+L2)*Redx2)     
...
!----- 並列計算終わり ------
 ...
 !----- irep>iInitialStep のとき，巨視量を計算する -----
  ...
  !--- データの出力 ---
  write(filedata,'("Data(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
