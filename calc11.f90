...
! ・PorousDataRaw(pt,dpdx,porosity).dat
! 圧力勾配,空隙率,全セルの運動量の和,右側境界の質量流量,左側境界の質量流量,多孔質の表面積
! ・porous.dat
...
! 2016.7.28 出力ファイルのiPatternを8桁に変更
! 2016.8.16 PorousDataRaw(pt,dpdx,porosity).dat に生データを出力
!------------------------------------------------------------------------------------------
...
program calc
  ...
  write(fileinidata,'("KnudsenData(CN1_",i2,",CN2_",i2,",pt",i8,").dat")') &
       iCellNumber1,iCellNumber2,iPattern
  write(fileflu,'("Flux(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
  write(filemac,'("Macroscopic(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
  write(filedis1,'("DistributionFunc1(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
  write(filedis2,'("DistributionFunc2(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
  write(filedataraw,'("DataRaw(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
       Rho1,Tau1,iPattern
  ! write(filebound,'("BoundsNumber(Rho1_",f4.2,",Tau1_",f4.2,",pt",i8,").dat")') &
  !      Rho1,Tau1,iPattern
 ...
 !--- 物体のパターン，圧力勾配，空隙率に関するデータ
  write(filename,'("PorousData(pt",i8,",dpdx_",f5.2,",poro_",f4.2").dat")') &
       iPattern,grad_p,porosity
  write(filename2,'("PorousDataRaw(pt",i8,",dpdx_",f5.2,",poro_",f4.2").dat")') &
       iPattern,grad_p,porosity
  open(40,file=filename,status='replace')
  open(45,file=filename2,status='replace')
...
!------------------------------------------------------------------------------------
  ...
  write(45,*) mom_t
  write(45,*) mom_b
  write(45,*) mom_l + mom_r + mom_t + mom_b
  write(45,*) p_l
  write(45,*) p_r
  write(45,*) p_l-p_r
  write(45,*) p_t
  write(45,*) p_b
  write(45,*) p_l + p_r + p_t + p_b 
  write(45,*) mom_l + mom_r + mom_t + mom_b - p_l - p_r - p_t - p_b

  write(45,*) rho_r
  write(45,*) t_r

  write(45,*) average_p_left
  write(45,*) average_p_right
  write(45,*) average_p_right - average_p_left
